...
import { Sparkles, Clock, Flame, Star, Play, ShoppingCart, Plus, CircleDot, AlertCircle, ChefHat, Snowflake, ScrollText, CheckCircle2 } from "lucide-react";
...
function Dashboard() {
  ...
  useEffect(() => {
    if (!group) return;
    recipeService.suggest(group.id).then((s) => setSuggestion(s[0] ?? null));
    fridgeService.list(group.id).then(setFridge);
    shoppingService.list(group.id).then(setLists);
    mealPlanService.list(group.id).then(setMeals);
  }, [group]);
  ...
  async function addMissingToShopping() {
    if (!suggestion || !list) return;
    for (const name of suggestion.missing) {
      await shoppingService.addItem(list.id, { name, quantity: 1, unit: "gói", category: "Gia vị" });
    }
  ...
  return (
    <div className="space-y-6">
      {/* HERO */}
      <PageHero title={`Chào ${user?.name ?? "bạn"}! 👋`} subtitle={`Tủ lạnh: ${fridge.length} món • ${expiring.length} sắp hết hạn • 3 món gợi ý hôm nay`} />

      {/* SUGGESTION CARD */}
      {suggestion && (
        <section className="rounded-3xl bg-gradient-to-br from-primary to-primary-deep p-6 md:p-8 text-primary-foreground shadow-elevated">
          <div className="grid gap-6 md:grid-cols-[1fr_auto_auto] md:items-center">
              ...
              <div className="mt-4 space-y-1.5 text-sm">
                {suggestion.have.length > 0 && (
                  <div className="flex items-start gap-2">
                    ...
                    <span><span className="opacity-80">Có sẵn:</span> {suggestion.have.join(", ")}</span>
                  </div>
                )}
                {suggestion.missing.length > 0 && (
                  <div className="flex items-start gap-2">
                    <AlertCircle className="mt-0.5 h-4 w-4 text-destructive" />
                    <span><span className="opacity-80">Còn thiếu:</span> {suggestion.missing.join(", ")}</span>
                  </div>
                )}
              </div>
              <div className="mt-5 flex flex-wrap gap-2">
                <Button onClick={addMissingToShopping} className="bg-warning hover:bg-warning/90 text-warning-foreground font-semibold rounded-xl px-5 h-11">
                  Xem công thức →
                </Button>
                <Button variant="outline" className="bg-white/10 border-white/30 hover:bg-white/20 text-primary-foreground rounded-xl px-5 h-11">
                  Thêm thực đơn
                </Button>
              </div>
            </div>
            ...
            {suggestion.recipe.image && (
              <div className="relative overflow-hidden rounded-2xl shadow-elevated md:w-[320px]">
                ...
                <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                  <div className="text-white font-bold drop-shadow">{suggestion.recipe.name}</div>
                </div>
                <button className="absolute bottom-3 right-3 inline-flex items-center gap-1.5 rounded-full bg-success px-3 py-1.5 text-xs font-semibold text-success-foreground">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
        {/* Meal plan today */}
        <div className="rounded-3xl bg-accent/40 p-6 shadow-card">
          <div className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Meal Plan</div>
          ...
          <div className="mt-4 text-center">
            <Link to="/meal-plan" className="inline-flex items-center gap-1 text-sm font-semibold text-primary hover:text-primary-deep">
        ...
        <div className="space-y-4">
          {list && (
            <div className="rounded-3xl bg-card p-6 shadow-card">
                ...
                return (
                  <>
                    ...
                    <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-secondary">
                      <div className="h-full rounded-full bg-gradient-to-r from-info to-primary transition-all" style={{ width: `${pct}%` }} />
                    </div>
                  </>
                );
              })()}
              <ul className="mt-4 space-y-2">
                {list.items.slice(0, 5).map((it) => (
                  <li key={it.id} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 ${it.bought ? "bg-secondary/60" : "bg-secondary/30"}`}>
                    <div className={`grid h-6 w-6 place-items-center rounded-full ${it.bought ? "bg-success text-success-foreground" : "border-2 border-border bg-card"}`}>
                      {it.bought && <CheckCircle2 className="h-4 w-4" />}
                    </div>
              ...
              <Link to="/shopping" className="mt-4 block w-full rounded-xl bg-gradient-to-r from-info to-primary py-3 text-center text-sm font-semibold text-white hover:opacity-90 transition">
                Xem tất cả →
              </Link>
            </div>
          )}
                  ...
                  return (
                    <div key={f.id} className={`flex items-center gap-3 rounded-xl p-3 ${d <= 2 ? "bg-destructive/10" : "bg-warning/10"}`}>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-muted-foreground">Hết hạn: {fmtDate(f.expiryDate)}</div>
                      </div>
                      <span className={`rounded-md px-2 py-1 text-xs font-bold text-white ${d <= 2 ? "bg-destructive" : "bg-warning"}`}>
                        {Math.max(0, d)} ngày
                      </span>
                    </div>
                  );
      ...
      <div className="rounded-3xl bg-card p-6 shadow-card">
        <div className="flex items-center justify-between">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-accent px-3 py-1 text-xs font-semibold text-accent-foreground">
            <CircleDot className="h-2 w-2 fill-current" /> Family feed
          </span>
        ...
        <div className="mt-5 relative space-y-4 pl-12">
          ...
          {feed.map((f) => (
            <div key={f.id} className="relative">
              <div className="absolute -left-[34px] top-1 grid h-9 w-9 place-items-center rounded-xl bg-card border-2 border-border">
                <FeedIcon kind={f.kind} />
              </div>
              <div className="rounded-2xl bg-secondary/40 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-warning">{f.userName}</span>
                    <span className="rounded-md bg-secondary px-2 py-0.5 text-xs text-muted-foreground">thành viên</span>
                  </div>
                  <span className="text-xs text-muted-foreground">{fmtRelative(f.createdAt)}</span>
                </div>
                <p className="mt-1.5 text-sm">{f.message}</p>
                <div className="mt-2 flex gap-2 text-xs">
                  <span className="inline-flex items-center gap-1 rounded-full bg-card px-2.5 py-1 text-muted-foreground">❤ Hoạt động gia đình</span>
                  <span className="inline-flex items-center gap-1 rounded-full bg-card px-2.5 py-1 text-muted-foreground">Bình luận</span>
                </div>
              </div>
            </div>
          ))}
