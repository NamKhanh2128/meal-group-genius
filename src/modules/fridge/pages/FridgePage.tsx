...
import { BackButton, RefreshButton } from "@/components/common/PageActions";
...
export function FridgePage() {
  ...
  const { items, load, remove, loading } = useFridgeStore();
  ...
  return (
    <>
      <ScreenHeader
        ...
        actions={<div className="flex flex-wrap gap-2"><BackButton /><RefreshButton onClick={() => void load(family.family_id)} /><Button asChild className="rounded-[8px] bg-[#ffb11f]"><Link to="/fridge/add"><Plus className="mr-2 h-4 w-4" />Thêm thực phẩm</Link></Button></div>}
      />
      ...
      <section className="rounded-[8px] bg-white p-5 shadow-card">
        ...
        <div className="mt-5 hidden overflow-hidden rounded-[8px] border lg:block">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((item) => <tr key={item.fridge_item_id} className="border-t"><td className="p-3 font-bold">{item.food.icon} {item.food.food_name}</td><td>{item.quantity}</td><td>{item.food.unit}</td><td><span className={daysUntil(item.expiry_date) <= 3 ? "font-bold text-red-600" : ""}>{formatDate(item.expiry_date)}</span></td><td>{item.food.category}</td><td>{item.location}</td><td className="space-x-2 pr-3 text-right"><Button asChild size="sm" variant="outline"><Link to={`/fridge/edit/${item.fridge_item_id}`}><Edit className="h-4 w-4" /></Link></Button><Button size="sm" variant="outline" onClick={() => toast.info("Consume flow: nhập lượng đã dùng rồi cập nhật fridge_items.")}><Utensils className="h-4 w-4" /></Button><Button size="sm" variant="destructive" onClick={() => setDeleteId(item.fridge_item_id)}><Trash2 className="h-4 w-4" /></Button></td></tr>)}
