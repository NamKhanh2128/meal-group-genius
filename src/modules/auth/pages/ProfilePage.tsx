...
export function ProfilePage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<Values>({ resolver: zodResolver(profileSchema), defaultValues: { full_name: user.full_name, email: user.email, phone: user.phone ?? "", avatar_url: user.avatar_url ?? "" } });
  ...
  return (
    <>
      <ScreenHeader title="Hồ sơ người dùng" subtitle={`Gia đình hiện tại: ${family?.family_name ?? "Chưa có"}`} actions={<BackButton />} />
      ...
      <AppModal open={passwordOpen} onOpenChange={setPasswordOpen} type="confirm" title="Change password" primaryLabel="Cập nhật" secondaryLabel="Hủy" onPrimary={passwordForm.handleSubmit(submitPassword)}>
        <div className="space-y-3">
          <Input type="password" placeholder="old password" {...passwordForm.register("old_password")} />
          ...
          <Input type="password" placeholder="new password" {...passwordForm.register("new_password")} />
          ...
          <Input type="password" placeholder="confirm" {...passwordForm.register("confirm_password")} />
