import { AlertCircle, CheckCircle2, Clock, Flame, Play, Plus, Refrigerator, ScrollText, ShoppingCart, Star } from "lucide-react";
...
export function DashboardPage() {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      familyApi.detail(family.family_id).then((data) => { setActivities(data.activities); setMembers(data.members); }),
    ]);
  }, [family.family_id]);
  ...
  const boughtCount = activeList?.items.filter((item) => item.bought_status).length ?? 0;
    ...
    <div className="-mx-4 -my-7 min-h-[calc(100vh-68px)] bg-[#66429c] px-4 pb-12 pt-8 sm:px-8">
      <section className="mx-auto max-w-[1324px]">
            ...
            <div className="relative grid gap-7 lg:grid-cols-[1.25fr_180px_445px] lg:items-center">
              <div>
                ...
                <div className="mt-7 flex flex-wrap gap-3">
                  ...
                  <Button asChild variant="outline" className="h-12 rounded-[10px] border-white/35 bg-white/8 px-7 font-bold text-white hover:bg-white/16"><Link to="/meal-planner">Lên kế hoạch</Link></Button>
                </div>
              </div>
              ...
              <div className="relative overflow-hidden rounded-[20px] bg-black shadow-[0_22px_45px_rgba(0,0,0,0.3)]">
                ...
                <button className="absolute bottom-8 left-6 inline-flex items-center gap-2 rounded-full bg-[#22c972] px-5 py-2 text-sm font-bold"><Play className="h-4 w-4 fill-current" /> Xem video hướng dẫn</button>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-3">
              {mealCards.map(({ meal, recipes: mealRecipes }) => (
                <div key={`${meal.meal_date}-${meal.meal_type}`} className="overflow-hidden rounded-[12px] bg-white shadow-sm">
                  <div className="relative h-140px h-36">
                    ...
                    <span className="absolute bottom-3 right-3 rounded-full bg-[#36c977] px-3 py-1 text-xs font-bold text-white">{meal.meal_type === "Sáng" ? "Đã xong" : meal.meal_type === "Trưa" ? "Đang nấu" : "Kế hoạch"}</span>
                  ...
                  <div className="space-y-1 p-3 text-xs text-[#766d86]">
                    <div>👥 {meal.meal_type === "Sáng" ? "Đã nấu" : "Đã lên kế hoạch"} · {members.length || 1} người</div>
                    <div>⏱ {meal.meal_type === "Trưa" ? "11:30" : meal.meal_type === "Tối" ? "18:00" : "Chưa đặt giờ"}</div>
                  </div>
                </div>
              ))}
            </div>
            <Link to="/meal-planner" className="mt-6 flex items-center justify-center gap-2 text-sm font-bold text-[#7655aa]"><Plus className="h-4 w-4" /> Lên kế hoạch bữa ăn</Link>
          </section>
              ...
              <section className="rounded-[20px] bg-white p-6 shadow-card">
                ...
                <div className="mt-6 space-y-3">
                  {activeList.items.slice(0, 5).map((item) => (
                    <div key={item.id} className={`flex items-center gap-3 rounded-[12px] px-4 py-3 ${item.bought_status ? "bg-[#f0f2f7] text-[#9a93a6] line-through" : "bg-white ring-1 ring-[#ebe8f0]"}`}>
                      <span className={`grid h-6 w-6 place-items-center rounded-full ${item.bought_status ? "bg-[#31c875] text-white" : "border border-[#d4d0dc]"}`}>{item.bought_status && "✓"}</span>
        ...
        <section className="mt-6 rounded-[20px] bg-white p-6 shadow-card lg:w-[calc(100%-458px)]">
          ...
          <p className="text-sm text-[#746d82]">Cập nhật mới từ các thành viên trong nhà</p>
          <div className="relative mt-6 space-y-4 pl-14">
            ...
            {activities.map((activity) => {
              ...
              return (
                <div key={activity.id} className="relative rounded-[14px] border border-[#eee9f5] p-4">
